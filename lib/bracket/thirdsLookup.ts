import type { AnnexCRow, GroupLetter } from "@/types/bracket";

// Annex C — FIFA World Cup 2026 Competition Regulations
// Each entry: sorted 8-group key → [T1,T2,T3,T4,T5,T6,T7,T8] group assignments
// T1=1A-vs, T2=1B-vs, T3=1D-vs, T4=1E-vs, T5=1G-vs, T6=1I-vs, T7=1K-vs, T8=1L-vs
// Source: en.wikipedia.org/wiki/Template:2026_FIFA_World_Cup_third-place_table
// 495 combinations — all validated against pool constraints
const ANNEX_C: Record<string, readonly [string,string,string,string,string,string,string,string]> = {
...
const SLOT_IDS = ["R32_T1","R32_T2","R32_T3","R32_T4","R32_T5","R32_T6","R32_T7","R32_T8"] as const;

function rowKey(groups: GroupLetter[]): string {
  return [...groups].sort().join("");
}

/**
 * Given the 8 qualifying group letters, returns the FIFA Annex C assignment
 * mapping each R32_T* slot ID to the group whose 3rd-place team fills it.
 * Returns null when the combination is not in the table.
 */
export function getThirdAssignments(
  qualifyingGroups: GroupLetter[]
): AnnexCRow["assignments"] | null {
  if (qualifyingGroups.length !== 8) return null;
  const row = ANNEX_C[rowKey(qualifyingGroups)];
  if (!row) return null;
  const assignments: AnnexCRow["assignments"] = {};
  for (let i = 0; i < SLOT_IDS.length; i++) {
    assignments[SLOT_IDS[i]] = row[i] as GroupLetter;
  }
  return assignments;
}